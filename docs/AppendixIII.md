Appendix III — DP aggregation output format, pseudocode, worked traces, and reproducibility notes
...
•	m = number of comparable relations (Hasse edges).
•	Indexing convention: Use 1-based positions in text and human-facing outputs; use 0-based arrays in code only where language-native arrays require it, and document that choice in README and code comments. All BlockSummary examples in this appendix use 1-based positions.
________________________________________
DP aggregation output format (machine- and human-readable)
BlockSummary(index = i) — field types
•	index: integer (1-based)
•	total_linear_extensions: integer
•	block_counts: list of [block_size:int, count:int] pairs where block_size ≥ 1
•	contributions: list of [block_size:int, count:int, contribution:int] triples with contribution = block_size * count
•	verification_sum: integer (sum of contributions; must equal total_linear_extensions)
...
•	block_size = number of absolute slots that element i may occupy in a full extension when that reduced-permutation class occurs.
•	count = number of reduced permutations (linear extensions of P \ {i}) producing that block_size.
•	contribution = block_size × count; summing contributions across block_sizes yields total_linear_extensions for index i.
________________________________________
Pseudocode
Notes:
•	P \ {i} denotes the poset with element i removed.
•	A reduced permutation is a linear extension of P \ {i}.
•	The DP-aggregation routine compresses many reduced permutations that share identical p and s-derived block sizes into aggregated counts.
...
•	The key compression decision is the design of state_signature: it must preserve exactly the information needed to compute p and s for final classes, and no more. Document the chosen signature format in the README.
•	For correctness-first development, run the enumerator to produce ground truth block_counts for small instances; use those results to refine state_signature choices.
•	Precompute transitive-closure/reachability once and reuse results to compute forced predecessors/successors cheaply.
•	Include unit tests that compare dp_aggregate output to enumerate_reduced_extensions + process_reduced_permutation for small instances.
________________________________________
...
Note: the single “Representative reduced permutation” shown below is an example illustrating how p and s are computed; aggregated block_counts are obtained by repeating process_reduced_permutation over all reduced permutations of P \ {i}.
Worked trace for index = 1 (socks)
Representative reduced permutation: R = [3,4,5,6,7,8,2] # permutation of P \ {1}
1.	forced predecessors of 1 in R: none ⇒ p = 0
2.	forced successors of 1 in R: {2} occurs at position 7 in R ⇒ s = 7
3.	incomparable block positions p+1..s−1 = 1..6 → elements [3,4,5,6,7,8] ⇒ k = 6
4.	block_size = k+1 = 7 → increment block_counts[7] by 1 → contribution +7
Repeat for all reduced permutations; final aggregated block_counts (index = 1):
•	block_size 3: count 6 → contribution 18
•	block_size 4: count 18 → contribution 72
•	block_size 5: count 30 → contribution 150
•	block_size 6: count 38 → contribution 228
•	block_size 7: count 38 → contribution 266
...
1.	forced predecessors of 5: {3,6} appear at positions 3 and 5 in R ⇒ p = 5
2.	forced successors of 5: none ⇒ s = len(R)+1 = 8
3.	incomparable block positions p+1..s−1 = 6..7 → elements [7,8] ⇒ k = 2
4.	block_size = k+1 = 3 → increment block_counts[3] by 1 → contribution +3
Final aggregated block_counts (index = 5):
•	block_size 2: count 40 → contribution 80
•	block_size 3: count 62 → contribution 186
•	block_size 4: count 72 → contribution 288
•	block_size 5: count 36 → contribution 180
...
Indexing
•	Human-facing tables and figures use 1-based positions.
•	Machine-readable JSON and CSV files use the documented indexing in the repository README and schema.
Required field names (use these exact keys)
•	total_linear_extensions, block_counts, contributions, verification_sum, per_run_file, wall_seconds, cpu_seconds, peak_mem_mb, seed, repeat, exit_code, notes, commit_hash, host.
Repository artifacts referenced in this appendix
•	results/aggregated_results.csv
•	results/per_run/*.json (BlockSummary JSONs per Appendix III schema)
•	results/logs/*.log and *.time.txt
•	results/plots/*.(png|pdf) (high-resolution raster and vector versions)
•	results/tables/*.csv (numeric summaries)
•	results/scripts/*.py (aggregation, verification, plotting scripts)
•	metadata/commit_hash.txt, metadata/checksums.txt, run_manifest.txt, CITATION.
...
•	Repository URL/DOI, exact commit hash, and access status are recorded in metadata/commit_hash.txt. Reviewers should use those identifiers to retrieve the archival snapshot referenced in this appendix.
Notes on verification and machine readability
•	BlockSummary JSON files conform to the schema given in Appendix III; verify_block_summaries.py validates verification_sum against total_linear_extensions and exits nonzero on mismatches.
•	aggregated_results.csv contains one row per run and includes a per_run_file column linking each row to its BlockSummary JSON for traceability.
________________________________________
Implementation Notes
